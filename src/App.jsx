  ...
  return (
    <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
      ...
      <nav className="primary-nav" aria-label="Primary navigation">
        ...
        <button aria-label="Help & feedback" className="nav-item mobile-nav-item" onClick={onHelp}>
...
function Overview({ project, onEdit, onDelete, onUpdate, onNavigate, notify }) {
  ...
  return (
    <div className="content real-dashboard">
      ...
      <div className="source-boundary"><ShieldCheck size={16} /><p><b>No synthetic records.</b> {project.catalogue ? "Calculated values use your inputs and the attached catalogue." : "Calculated values currently use your supplied project inputs only."} Missing signals remain unavailable.</p></div>
...
function SettingsView({ workspace, setWorkspace, notify, encryptionEnabled, onEnableEncryption, onDisableEncryption }) {
  ...
  const paymentLinkValid = !profile.stripePaymentLink || /^https:\/\/buy\.stripe\.com\//i.test(profile.stripePaymentLink.trim());
  const save = () => {
    if (!paymentLinkValid) return notify("Use a valid https://buy.stripe.com/ Payment Link.", "error");
  ...
  const importBackup = async (file) => {
    ...
    try {
      ...
      if (workspace.projects.length && !window.confirm("Replace the current workspace with this backup? Existing local projects will be removed. Export a backup first if you may need them.")) return;
